...
The number of parameters which a model has is deeply intertwined with its model architecture, we cannot change these without drastically effecting the model performance. The only way that we can possibly make the model's memory footprint small would be to somehow change the amount of memory per parameter that the model takes. 
